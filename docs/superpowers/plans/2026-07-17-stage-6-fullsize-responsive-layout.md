...
**Goal:** Заменить сжатый абсолютный HUD 1280×720 полноразмерным адаптивным интерфейсом 1920×1080, добавить обнаруживаемое масштабирование камеры и разрешить носильщикам проходить друг через друга без изменения статических препятствий.

**Architecture:** Главная сцена получает полноэкранный контейнерный каркас `TopBar + Body`, где `Body` состоит из левой панели, расширяемого центра и правого инспектора. `ResponsiveLayoutController` передаёт безопасный прямоугольник в единый `CameraController`, обслуживающий колёсико и HUD-кнопки. Из `SimulationState` и `MovementSystem` удаляется динамическая занятость клеток носильщиками; проходимость по-прежнему задают карта и здания.
...
- Recorder и хеш симуляции не зависят от layout; хеш обновляется согласованно после удаления динамической занятости.
- Масштаб камеры доступен колёсиком и кнопками `−`, `+`, `Вписать`, диапазон `50–200%`.
- Носильщики не блокируют и не резервируют клетки друг для друга; здания и непроходимая местность остаются препятствиями.
...
- Produces: видимые элементы `ZoomOutButton`, `ZoomLabel`, `ZoomInButton`, `ZoomFitButton`.
...
Set hint/target to word-smart autowrap, make the copy column expand instead of fixed `540 px`, and give all six tool buttons equal horizontal expansion. Add the compact zoom group to `TopBar`; it calls `CameraController` and never stores an independent zoom value. Keep `ResultPanel` as a centered modal sibling of `SafeArea`.
...
### Task 4: Сквозное движение носильщиков

**Files:**
- Modify: `src/simulation/model/simulation_state.gd`
- Modify: `src/simulation/loading/scenario_loader.gd`
- Modify: `src/simulation/systems/movement_system.gd`
- Modify: `src/simulation/systems/path_system.gd`
...
- Modify: affected unit, scenario and helper files under `tests/`

**Interfaces:**
- Removes: `SimulationState.worker_occupancy`, `SimulationState.cell_reservations`.
- Preserves: static checks through `HexCellState.traversable` and `SimulationState.occupied_cells`.

- [ ] **Step 1: Replace collision expectations with pass-through tests**

Add RED assertions that two workers may begin movement into one target cell, may exchange cells in opposite directions, and still cannot enter a building or non-traversable cell. Update invariant expectations so worker overlap is valid.

- [ ] **Step 2: Run movement, invariant and loader suites to verify RED**

Expected: current reservation logic rejects the shared target and swap.

- [ ] **Step 3: Remove dynamic worker blocking**

Delete occupancy/reservation state, loader bookkeeping, release helpers and hash sections. `MovementSystem` processes workers by stable ID and checks only map traversal and `occupied_cells`; repath never receives workers as dynamic blockers.

- [ ] **Step 4: Update affected fixtures and deterministic hash tests**

Remove manual occupancy setup from helpers and scenario tests. Verify that reversing all remaining dictionaries leaves the hash unchanged.

- [ ] **Step 5: Run targeted suites to verify GREEN**

Run movement, path, logistics link, invariant, scenario loader, depot, workforce, transfer flow and state hasher suites.

- [ ] **Step 6: Commit**

```bash
git add src/simulation tests
git commit -m "feat: разрешить носильщикам проходить друг через друга"
```

### Task 5: Недействительный PT-V01 и новая серия
...
### Task 6: Приёмка полноразмерного прототипа
...
Run responsive layout, camera, movement pass-through, stage 5 UI, stage 6 integration and docs suites. Expected: all pass without script errors.
