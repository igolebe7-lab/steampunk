...
func run() -> Array[String]:
    ...
    assert_true(
        hasher.canonicalize(first).begins_with("v=6|"),
        "каноническое представление должно использовать формат v=6"
    )
    assert_eq(
...
func _assert_stage4_state_changes_hash(
    ...
    assert_true(initial_hash != hasher.hash_state(changed_road), "уровень дороги должен менять v=6 hash")

    var changed_revision := ScenarioLoader.new().load_scenario(scenario).state
    changed_revision.revision = 1
    assert_true(initial_hash != hasher.hash_state(changed_revision), "ревизия должна менять v=6 hash")
    ...
    assert_true(initial_hash != hasher.hash_state(changed_link), "логистическая связь должна менять v=6 hash")
    ...
    assert_true(initial_hash != hasher.hash_state(changed_slots), "полная таблица рабочих мест должна менять v=6 hash")
    ...
    assert_true(initial_hash != hasher.hash_state(changed_ports), "совместимость логистических портов должна менять v=6 hash")
...
func _assert_logistics_dictionary_order_is_ignored(hasher: StateHasher) -> void:
    ...
    assert_eq(
        ...
        expected,
        "порядок всех logistics Dictionary и flows не влияет на v6 hash"
    )
