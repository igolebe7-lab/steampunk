...
func run() -> Array[String]:
    ...
    assert_eq(instance.get_node("UI/TopBar/Margin/HBox/Title").text, "Паровая логистика", "заголовок должен быть локализован")
    assert_eq(instance.get_node("UI/TopBar/Margin/HBox/Status").text, "Выберите гекс", "статус должен быть локализован")
    instance.free()
    ...
    assert_eq(
        english_instance.get_node("UI/TopBar/Margin/HBox/Title").text,
        "Steam Logistics",
    ...
    assert_eq(
        english_instance.get_node("UI/TopBar/Margin/HBox/Status").text,
        "Selected hex: 3, 4",
