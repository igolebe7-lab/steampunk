...
func enqueue(command: SimulationCommand, completed_tick: int) -> CommandResult:
    ...
    if command.target_tick <= completed_tick:
        return CommandResult.rejected(&"past_tick", command.id)

    var command_key := command.id
    if _command_keys.has(command_key):
        return CommandResult.rejected(&"duplicate_sequence", command.id)
    ...
    tick_commands.append(command.snapshot())
...
func take_for_tick(tick: int) -> Array[SimulationCommand]:
    ...
    for command in result:
        _command_keys.erase(command.id)
