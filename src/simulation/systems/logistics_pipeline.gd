...
var _utility_network_system := UtilityNetworkSystem.new()
var _production_system := ProductionSystem.new()
var _scenario_system := ScenarioSystem.new()
...
func run(state: SimulationState, target_tick: int) -> void:
    ...
    _utility_network_system.run(state, target_tick)
    _production_system.run(state, target_tick)
    _scenario_system.run(state, target_tick)
